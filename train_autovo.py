...
loader = data.DataLoader(dataset=AutoVoDataset(), batch_size=batch_size, shuffle=True, num_workers=0)
...
def sample(net, audio, imgs, epoch_count):
    def tensor_to_img(x):
    ...
    net.eval()
    with torch.no_grad():
        recon = net(audio, imgs, vae)
        x_img = tensor_to_img(imgs.squeeze(0))
        recon_img = tensor_to_img(recon.squeeze(0))
...
if __name__ == '__main__':
    ...
    for epoch_count in range(1, 1 + epoch):
        ...
        for audio, imgs in loader:
            ...
            # print(audio.shape, imgs.shape)
            loss = model(audio, imgs, vae)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

            if loss_val == 0:
                sample(model, audio.detach(), imgs.detach(), epoch_count)

            loss_val += loss.item()
            processed += audio.shape[0]
